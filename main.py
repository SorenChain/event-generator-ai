...
from app.utils.mongodb_logging import setup_mongodb_logging, LoggedFunction, log_database_operation, cleanup_all_logs
...
async def process_sport_events(category_name, category_id):
    ...
    with LoggedFunction("process_sport_events", logger, category=category_name):
        ...
        try:
            ...
            logger.info(f"Retrieved {len(organized_events)} team events and {len(null_team_events)} null team events")
            ...
            for event in organized_events:
                try:
                    ...
                    if not (generated_question and end_date):
                        logger.warning(f"Skipping sport event: No valid question generated for topic {event['topic']}")
                        continue
                    ...
                    if prob2 is not None:
                        ...
                        if await save_event(event_data):
                            ...
                            logger.info(f"Binary sports event saved successfully: {generated_question}")
                        else:
                            logger.error(f"Failed to save binary sports event: {generated_question}")
                            
                except Exception as e:
                    logger.error(f"Error processing team event {event.get('topic', 'unknown')}: {str(e)}", exc_info=True)
                    continue
            ...
            for event in null_team_events:
                try:
                    ...
                    logger.error(f"Error processing null team event {event.get('topic', 'unknown')}: {str(e)}", exc_info=True)
                    continue
            
            logger.info(f"Sports events processing completed. Team events: {team_events_processed}, Null team events: {null_events_processed}")
...
async def process_null_team_event(event, category_id):
    ...
    with LoggedFunction("process_null_team_event", logger, event_key=event.get('key')):
        ...
        if not (generated_question and event.get('formatted_date')):
            logger.warning(f"Skipping null team event: No valid question generated for {event.get('topic')}")
            return False
        ...
        for option in prob1:
            try:
                ...
                if binary_save_response:
                    ...
                    logger.info(f"Binary event for option '{option_name}' saved with ID: {binary_event_id}")
                    ...
                    logger.error(f"Failed to save binary event for option '{option_name}'")
                    
            except Exception as e:
                logger.error(f"Error processing option '{option.get('option', 'unknown')}': {str(e)}", exc_info=True)
                continue
        ...
        if updated_options:
            try:
                ...
                if update_result.modified_count > 0:
                    ...
                    logger.info(f"Parent event '{parent_event_id}' updated with {len(updated_options)} options successfully.")
                else:
                    logger.error(f"Failed to update parent event '{parent_event_id}' with options.")
                    return False
            except Exception as e:
                logger.error(f"Error updating parent event with options: {str(e)}", exc_info=True)
                return False
        
        logger.info(f"Null team event processed successfully. Options created: {options_processed}")
        return True
...
async def process_regular_event(event):
    ...
    with LoggedFunction("process_regular_event", logger, 
                       url=row['Link'], 
                       category=event['category_name'], 
                       topic=event['topic_name']):
        
        logger.info(f"Processing URL: {row['Link']} (Category: {event['category_name']}, Topic: {event['topic_name']})")
        
        try:
            # Get event content
            event_description = await document_loader(row['Link'])
            if event_description is None:
                logger.warning(f"No content retrieved for URL: {row['Link']}. Skipping.")
                return False
            ...
            logger.error(f"Error processing event for URL {row['Link']}: {str(e)}", exc_info=True)
            return False
...
async def save_binary_event(event, question, prob1, prob2, end_date, description, row):
    ...
    with LoggedFunction("save_binary_event", logger, question=question):
        try:
            ...
            logger.error(f"Error saving binary event '{question}': {str(e)}", exc_info=True)
            return False
...
async def save_multi_option_event(event, question, options, end_date, description, row):
    ...
    """
    with LoggedFunction("save_multi_option_event", logger, question=question, options_count=len(options)):
        try:
            ...
            for option in options:
                try:
                    ...
                    if binary_save_response:
                        ...
                        updated_options.append(option_data)
                        options_processed += 1
                        logger.info(f"Created binary event for option '{option_name}' with ID: {binary_event_id}")
                    else:
                        logger.error(f"Failed to save binary event for option '{option_name}'")
            ...
            logger.info(f"Multi-option event processed successfully. Options created: {options_processed}")
            return True
            
        except Exception as e:
            logger.error(f"Error saving multi-option event '{question}': {str(e)}", exc_info=True)
            return False
...
async def main():
    ...
    with LoggedFunction("main", logger):
        ...
        try:
            ...
            deleted_count = await cleanup_all_logs("prediction_market_logs", logger=logger)
            logger.info(f"Log cleanup completed. Deleted {deleted_count} log entries.")
            
        except Exception as e:
            logger.warning(f"Log cleanup failed: {str(e)}. Continuing with main process...")
        ...
        try:
            ...
            for category, topics in subreddits.items():
                try:
                    ...
                    if not topics:
                        logger.warning(f"No topics found for category '{category_name}'. Skipping.")
                        continue
                        
                    logger.info(f"Collecting events for category: {category_name} with {len(topics)} topics")
                    ...
                    for topic in topics:
                        try:
                            ...
                            logger.info(f"Searching events for topic '{topic_name}' using: {subreddit}")
                            ...
                            logger.info(f"Found {len(dataFrame)} events for topic '{topic_name}'")
                            await asyncio.sleep(5)  # Pause between topics
                            
                        except Exception as e:
                            logger.error(f"Error processing topic '{topic}' in category '{category_name}': {str(e)}", exc_info=True)
                            continue
            ...
            for i, event in enumerate(all_events, 1):
                try:
                    ...
                    failed_events += 1
                    logger.error(f"Failed to process event {i}: {str(e)}", exc_info=True)
                    continue
            
            logger.info(f"Data extraction and processing completed. Successful: {successful_events}, Failed: {failed_events}")
...
if __name__ == "__main__":
    try:
        ...
        logger.critical(f"Script failed with critical error: {str(e)}", exc_info=True)
    finally:
